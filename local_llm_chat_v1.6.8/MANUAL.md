...
3. `local_llm_chat_v1.6.8.html` をブラウザで開く
...
※ 🗑 Clearとの違い：Clearは画面も含めてすべて削除しますが、新しい話題は画面上の会話を保持したまま、AIの文脈のみをリセットします。
