...
3. `local_llm_chat_v1.6.6.1.html` をブラウザで開く
...
### ヘッダー
- **タイトル**: Local LLM Chat（右横に「Distributed at JRC2026」と表示）
- **モデル選択**: ロードされているモデルを選択（👁️マークはVision対応）
- **🔄 Refresh**: モデル一覧を再取得
- **🆕 新しい話題**: 会話コンテキストをリセット（画面上の会話は保持）
- **🗑 Clear**: 会話履歴をクリア
...
### バージョン表示（画面右下）
- 画面右下隅にバージョン番号（v1.6.6.1）が小さく表示されます
...
### 6. 新しい話題（v1.6.6.1新機能）
🆕 新しい話題ボタンで会話コンテキストをリセットできます：
- AIに送信される会話履歴がクリアされ、新しい話題として会話を開始
- 画面上の以前の会話はそのまま残るため、参照が可能
- セパレーターラインが表示され、話題の区切りが視覚的に分かる

※ 「🗑 Clear」との違い：Clearは画面上の会話もすべて消去しますが、「新しい話題」は画面上の表示を残したままコンテキストのみリセットします。
...
### v1.6.6.1 (2026-02-14)
- **新機能**
  - 🆕 新しい話題: 会話コンテキストをリセットして新しい話題を開始できる機能を追加（画面上の会話は保持）
- **バグ修正（v1.7.3からバックポート）**
  - 🔄 Regenerate機能: DOM要素から正確にユーザーメッセージを取得するよう修正（v1.6.7由来）
  - モデル一覧のソート: フルパスではなく表示名でソートするよう修正（v1.7.0由来）
  - ⏹ Stop時の履歴保存: 生成停止時にユーザーメッセージと途中の応答が履歴に保存されるよう修正（v1.7.1由来）
