 ...
 *   - デバッグログ機能
 *
 * v1.6.4 バグ修正 (2025-12-23):
 *   - Ctrl+Enter送信設定が効かない問題を修正
 *   - ストリーミング中のスクロール操作を可能に（スマートスクロール）
 *   - Stopボタンが押せない問題を修正
 *
 ...
 *   - モデル選択時にsettings.modelを即座に更新するように改善
 *   - API呼び出し時にUI上のモデル選択を優先するように変更
 *   - ヘルプモード: アプリの使い方をLLMに質問できる機能を追加
 *
 * v1.6.6.1 バグ修正・機能追加 (バックポート from v1.7.x):
 *   - 🆕 新しい話題ボタン: 会話コンテキストをリセット（画面は保持）（v1.6.8由来）
 *   - Regenerate機能: ストリーミングエラー後のRegenerate（🔄）が機能しない問題を修正（v1.6.7由来）
 *   - モデルリストソート: フルパスではなく表示名でアルファベット順にソート（v1.7.0由来）
 *   - 応答停止後のEdit/Regenerate: 停止時にユーザーメッセージと途中の応答を履歴に保存（v1.7.1由来）
 */
(() => {
  "use strict";
...
4. local_llm_chat_v1.6.6.html をブラウザで開く
...
### Q: 会話を他の人と共有できますか？
A: 💾 Exportで会話をJSONファイルとしてエクスポートし、共有できます。
...
`.trim();
  ...
  /**
   * 同じroleのメッセージが連続している場合、後のメッセージで上書きして正規化する
   * （ストリーミングエラー等で user,user や assistant,assistant が連続する場合の修復）
   */
  ...
  /**
   * 再生成: アシスタントメッセージを削除し、直前のユーザーメッセージを再送信する。
   * v1.6.6.1: UI上のメッセージコンテンツを参照して再送信するよう改善（v1.6.7由来）
   *   - ストリーミングエラー後でも履歴に未保存のメッセージをUI上から取得して再送信可能に
   * @param {HTMLDivElement} msgDiv
   */
...
上記のマニュアル内容を基に、ユーザーの質問に回答してください。`;
  ...
  async function refreshModels() {
    ...
    try {
      ...
      // アルファベット順にソート（表示名で、大文字小文字を区別しない）（v1.7.0由来）
      list.sort((a, b) => {
  ...
  async function handleSend() {
    ...
    let messagesSaved = false;                // ★ 二重保存防止フラグ
    ...
    try {
      ...
      await consumeSSE(
        ...
        () => {
          // onDone: UI最終化のみ（メッセージ保存はawait後に行う）
          const contentEl = currentMsgDiv.querySelector(".message-content");
      ...
      // ★ ストリーミング正常完了後にメッセージを保存（onDoneの外で行う）
      if (!messagesSaved) {
      ...
      if (e && e.name === "AbortError") {
        ...
        // ★ 停止時もユーザーメッセージと途中の応答を履歴に保存（Edit/Regenerate対応）（v1.7.1由来）
        ...
        const errorContent = currentContent + errorMsg;
        if (contentEl) contentEl.innerHTML = marked.parse(errorContent);
        currentMsgDiv.dataset.content = errorContent;
        console.error("Streaming error:", e);
        // ★ 部分的なコンテンツがある場合は履歴に保存（Edit/Regenerate対応）
        if (currentContent && !messagesSaved) {
          messages.push(userMessageForHistory);
          messages.push({ role: "assistant", content: errorContent });
          persistHistory();
          messagesSaved = true;
        }
      }
