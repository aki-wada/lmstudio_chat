...
不明な点がある場合は、推測せずにユーザーに確認してください。`;
  ...
  async function refreshModels() {
    ...
    try {
      ...
      buildModelDropdown(el.modelSelect, list);
      if (el.compareModelSelect) buildModelDropdown(el.compareModelSelect, list);

      // Selection: saved → first
      const preferred = state.settings.model;
      let chosen = null;
      for (const cand of [preferred, list[0]].filter(Boolean)) {
...
- **詳細タブ**: 信頼度・代替候補表示ON/OFF、モデル自動アンロードON/OFF、プリセット編集、データ管理（設定リセット/全データクリア）
...
- 必ず有効なJSONのみを返してください`;
  ...
  function init() {
    ...
    console.log("Local LLM Chat v2.0 initialized");
  }
