...
- 🔄 話題リセット: AIの記憶をリセットして新しい話題を開始（画面はそのまま）
- 🗑️ クリア: 画面の会話をすべて消去
- ❓: ヘルプモードのON/OFF
- ⚙️: 設定パネルを開く
...
### 話題リセット
🔄 話題リセットボタンをクリックすると、AIへの会話履歴送信がリセットされます。
...
### Q: 「話題リセット」と「クリア」の違いは？
A: 「話題リセット」は画面を保持したままAIの記憶のみリセット。「クリア」は画面の会話もすべて削除します。
...
`.trim();
  ...
  function showWelcomeScreen() {
    ...
    welcome.innerHTML = `
      ...
      <p class="welcome-version">v1.8.0 — JRC2026 Edition</p>
...
応答の生成の前に、まず質問内容を確認し、医学用語についてユーザの入力に不備があれば、ユーザに確認してください。`,
  ...
  /**
   * 新しい話題を開始する
   * 画面の表示はそのまま保持し、LLMへ送信する会話コンテキストだけをリセットする
   */
  function startNewTopic() {
    ...
    // 現在のメッセージ数を境界に設定 → 以降のメッセージだけがAPIに送られる
    topicStartIndex = messages.length;

    // 区切り線を表示
    const divider = document.createElement("div");
    divider.className = "topic-divider";
    divider.textContent = "── 新しい話題 ──";
    el.chat.appendChild(divider);
    divider.scrollIntoView({ behavior: "smooth" });

    notify("🆕 AIの文脈をリセットしました（表示はそのまま）");
  }
