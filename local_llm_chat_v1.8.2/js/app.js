 ...
 *
 * v1.8.2 新機能 (2026-03-04):
 *   - 🌐 応答言語設定: 応答言語を自動/日本語/English/中文/한국어から選択可能
 *   - 📖 プロンプト集ボタン表示設定: ヘッダーの📖ボタン表示/非表示を設定で切替可能
 *   - 🛡️ marked.js CDNフォールバック: ローカル読込失敗時にCDNから自動取得
 *
 * v1.8.1 新機能 (2026-03-03):
 *   - 💭 思考プロセス非表示: <think>タグの思考プロセス折りたたみ表示/非表示を設定で切替可能
...
(() => {
  ...
  const DEFAULT_SETTINGS = Object.freeze({
    ...
    responseLanguage: "",         // v1.8.2: 応答言語（"" = 自動, "ja", "en", "zh", "ko"）
    sendKey: "enter",
    ...
    showSamplePrompts: true,  // v1.8.2: プロンプト集ボタンの表示
    hideThinking: false,      // v1.8.0: 思考プロセス表示を非表示
    disableThinking: false,   // v1.8.0: Thinkingモード無効化（Qwen等）
  });
  ...
  const el = Object.freeze({
    ...
    responseLanguage: document.getElementById("responseLanguage"),  // v1.8.2
    ...
    showSamplePromptsToggle: document.getElementById("showSamplePromptsToggle"), // v1.8.2
    hideThinkingToggle: document.getElementById("hideThinkingToggle"),       // v1.8.0
    disableThinkingToggle: document.getElementById("disableThinkingToggle"), // v1.8.0
    ...
    samplePromptsModal: document.getElementById("samplePromptsModal"),
    samplePromptsCloseBtn: document.getElementById("samplePromptsCloseBtn"),
    samplePromptsFrame: document.getElementById("samplePromptsFrame"),
...
## v1.8.1 新機能
- 💭 **思考プロセス非表示**: 設定でthinkingモデルの思考プロセス折りたたみ表示/非表示を切替可能
...
### Q: 会話をリセットしたい場合は？
A: 🗑️ クリアボタンで画面の会話を削除し、AIの記憶もリセットされます。
...
`.trim();
  ...
  function loadSettings() {
    ...
    return /** @type {Settings} */ ({
      ...
      responseLanguage: s.responseLanguage || DEFAULT_SETTINGS.responseLanguage,  // v1.8.2
      ...
      showSamplePrompts: s.showSamplePrompts !== false,  // v1.8.2: デフォルトtrue
      hideThinking: Boolean(s.hideThinking),       // v1.8.0
      disableThinking: Boolean(s.disableThinking), // v1.8.0
    });
  }
  ...
  function saveSettingsFromUI() {
    settings = {
      ...
      responseLanguage: el.responseLanguage?.value || "",  // v1.8.2
      ...
      showSamplePrompts: el.showSamplePromptsToggle?.checked !== false,  // v1.8.2
      hideThinking: el.hideThinkingToggle?.checked || false,       // v1.8.0
      disableThinking: el.disableThinkingToggle?.checked || false, // v1.8.0
    };
  ...
  function showWelcomeScreen() {
    ...
    welcome.innerHTML = `
      ...
      <p class="welcome-version">v1.8.2</p>
...
上記のマニュアル内容を基に、ユーザーの質問に回答してください。`;
      ...
      try {
        ...
        await consumeSSE(
          ...
          (delta, _reasoningDelta) => {
            ...
            const contentEl = msgEl.querySelector(".message-content");
            if (contentEl) {
              const { thinking: tk, main: mn, isPartial: ip } = extractThinkingBlocks(content);
              contentEl.innerHTML = renderThinkingHtml(tk, ip) + marked.parse(mn);
            }
            smartScrollToBottom();
          },
          () => {
            const contentEl = msgEl.querySelector(".message-content");
            if (contentEl) {
              const { thinking: tk, main: mn, isPartial: ip } = extractThinkingBlocks(content || "(空応答)");
              contentEl.innerHTML = renderThinkingHtml(tk, ip) + marked.parse(mn || "(空応答)");
            }
          }
        );
  ...
  async function handleSend() {
    ...
    // v1.8.1: Thinkingモード無効化 — ユーザーメッセージ末尾に /no_think を付与
    if (settings.disableThinking) {
      text += " /no_think";
    }
...
【文章】`,
  ...
  function setupKeyboardShortcuts() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      // Esc で各パネル/モーダルを閉じる
      if (e.key === "Escape") {
        if (el.samplePromptsModal && el.samplePromptsModal.style.display === "flex") {
          el.samplePromptsModal.style.display = "none";
          return;
        }
        if (el.settingsPanel.classList.contains("open")) {
          closeSettingsPanel();
  ...
  function wireSettingsEvents() {
    ...
    // v1.8.2: 応答言語
    if (el.responseLanguage) {
      el.responseLanguage.onchange = save;
    }
  ...
  function wireSamplePromptsEvents() {
    if (!el.samplePromptsBtn) return;
    const toggle = () => {
      const modal = el.samplePromptsModal;
      if (!modal) return;
      const opening = modal.style.display !== "flex";
      modal.style.display = opening ? "flex" : "none";
      if (opening && el.samplePromptsFrame && !el.samplePromptsFrame.src.includes("SAMPLE_PROMPTS")) {
        el.samplePromptsFrame.src = "SAMPLE_PROMPTS.html";
      }
    };
    el.samplePromptsBtn.onclick = toggle;
    if (el.samplePromptsCloseBtn) {
      el.samplePromptsCloseBtn.onclick = toggle;
    }
    if (el.samplePromptsModal) {
      el.samplePromptsModal.addEventListener("click", (e) => {
        if (e.target === el.samplePromptsModal) toggle();
      });
    }
  }
  ...
  async function init() {
    ...
    wireSamplePromptsEvents();   // ★ プロンプト集モーダル (v1.8.1)
