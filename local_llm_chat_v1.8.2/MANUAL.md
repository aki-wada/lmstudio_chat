...
**2026 JRC ハンズオンセミナー 配布資料** (v1.8.0)
...
- **タイトル**: Local LLM Chat v1.8.0
- **モデル選択**: ロードされているモデルを選択（👁️マークはVision対応）
  - クリック時に自動でモデル一覧を更新
- **⚖️ 比較**: モデル比較モードのON/OFF（v1.7.0新機能）
...
- **ファイル添付**: テキストファイルやPDFの内容を送信
  ...
  - PDF: テキスト抽出してLLMに送信（ページ数・文字数を通知）
  - **サイズ制限: テキストファイル2MB以下、PDF10MB以下**
  - **PDFテキスト上限: 約30,000文字**（超過時は先頭のみ読み込み、警告を表示）
  - 画像のみのPDF（スキャンPDF等）はテキスト抽出不可（警告表示）

### 3. Vision対応モデル表示

- ドロップダウンにVision対応モデルは👁️アイコン付きで表示
- 対応キーワード: vision, llava, gemma-3, pixtral, devstral, magistral, qwen-vl, qwen2-vl, qwen3-vl, bakllava, obsidian, moondream, minicpm-v, cogvlm, glm-4v, internlm-xcomposer, medgemma など
...
- AIへの会話履歴送信がリセットされます
- 画面上の会話はそのまま保持され、「── 新しい話題 ──」の区切り線が表示されます
- ユーザープロフィール（呼び名・職業・専門レベル等）は自動で引き継がれます
- 話題を変えた際に前の回答が繰り返されることを防ぎます

※ 🗑️ クリアとの違い：クリアは画面の会話もすべて削除しますが、話題リセットは画面を保持したまま、AIの記憶のみをリセットします。
...
### 8. モデル比較機能（オプション・24GB以上推奨）

⚖️ 比較ボタンで2つのモデルの回答を並べて比較できます。2つのモデルを同時にロードするため、十分なメモリ（24GB以上）がある環境で使用してください。

**使い方:**

1. ⚖️ 比較ボタンをクリックして比較モードをONにする
...
- Settings「🔄 モデル切替時に前のモデルを自動アンロード」がデフォルトONになっています
...
| 🧠 思考プロセスを折り畳む | `<think>`タグ出力を折りたたみ表示                            | OFF                      |
| 🔄 自動アンロード       | モデル切替時に前のモデルを自動アンロード（LM Studio v0.4以降） | ON                       |
| 送信キー                | メッセージ送信のキー設定                                       | Enter で送信             |
| 応答スタイル            | AIの応答の詳しさ（簡潔/標準/詳細/専門的）                      | 標準                     |
| Temperature             | 創造性（0=安定、2=創造的）                                     | 0.7                      |
| Max Tokens              | 最大出力トークン数                                             | 2048                     |
| Reasoning Effort        | 推論モデル用の推論レベル（Low/Medium/High）                    | 指定なし                 |
...
| 項目          | 説明                                               |
| ------------- | -------------------------------------------------- |
| 呼び名        | AIがユーザーを呼ぶ名前（例: 田中先生）             |
| 専門レベル    | 初心者〜専門家（AIの説明レベルが変わります）        |
| 職業/専門     | 例: 放射線科医、学生                                |
| 興味・関心    | 例: CT画像診断、機械学習                            |

※ 「話題リセット」してもプロフィールは自動で引き継がれます。
...
| プリセット | 説明 |
| --- | --- |
| 💬 汎用アシスタント | 一般的な質問応答（デフォルト） |
| 🩻 放射線科アシスタント | 放射線画像診断の専門アシスタント |

ユーザー定義のプリセットも保存・切替可能です。

**プリセット編集:** プリセットの新規追加・内容編集・削除が可能です。

### 👁️ モデルタブ

ドロップダウンに表示するモデルを選択できます。不要なモデルを非表示にしてリストを整理できます。

### 📂 データタブ

**会話ログ管理:**

会話はセッション（ログ）単位で自動保存されます。設定パネルの「📂 データ」タブで管理できます。

| 操作 | 説明 |
| --- | --- |
| **＋ 新規チャット** | 現在の会話を保存し、新しい空のセッションを開始 |
| **セッション一覧** | 過去の会話ログが一覧表示される。クリックで切り替え |
| **削除** | 各セッションの「削除」ボタンで個別に削除可能 |

- セッションは最大 **100件** まで自動保存されます（超過時は古いものから削除）
- セッションのタイトルは最初のメッセージから自動生成されます
- ブラウザの localStorage に保存されるため、同じブラウザ・同じPCでのみ参照できます
- 「🗑️」（履歴クリア）ボタンで現在のセッションの内容を空にできます

**データ管理:**

- **🔄 設定をデフォルトに戻す**: 設定のみリセット（会話ログは保持）
- **🗑 すべての保存データを消す**: 以下のデータをすべて削除します（確認ダイアログが2回表示されます）
  - 会話履歴・セッション管理データ
  - 設定（Temperature、応答スタイル、プロフィール等）
  - カスタムプリセット・プリセットラベル
  - System Promptプリセット
  - モデル表示フィルター設定
  - 入力中の下書き
...
### 24GB以上 Mac の運用指針（24GB / 32GB M4 MacBook Air等）

#### 推奨モデルサイズ

| モデル              | 量子化 | サイズ  | 24GB          | 32GB          |
| ------------------- | ------ | ------- | ------------- | ------------- |
| **7B / 8B**   | Q4_K_M | 4-5GB   | ⭐⭐⭐ 余裕   | ⭐⭐⭐ 余裕   |
| 7B / 8B             | Q8_0   | 8-9GB   | ⭐⭐⭐ 快適   | ⭐⭐⭐ 余裕   |
| **13B / 14B** | Q4_K_M | 8-9GB   | ⭐⭐ 良好     | ⭐⭐⭐ 余裕   |
| 13B / 14B           | Q8_0   | 14-16GB | ⚠️ 注意     | ⭐⭐ 良好     |
| **27B**       | Q4_K_M | 15-16GB | ⚠️ ギリギリ | ⭐⭐ 良好     |
| 32B                 | Q4_K_M | 18-20GB | ❌ 不可       | ⚠️ 注意     |
| 70B                 | 任意   | 35GB+   | ❌ 不可       | ❌ 不可       |
...
| 設定項目             | 24GB               | 32GB               |
| -------------------- | ------------------ | ------------------ |
| **context length**   | 4096〜8192         | 8192〜16384        |
| **parallel**         | 1〜2               | 1〜2               |
| **KV cache量子化**   | 有効推奨           | 有効推奨           |

#### 安全な組み合わせ

| モデル  | context | parallel | 24GB       | 32GB       |
| ------- | ------- | -------- | ---------- | ---------- |
| 7B Q4   | 8192    | 2        | ✅ 安全   | ✅ 余裕   |
| 8B Q8   | 4096    | 2        | ✅ 安全   | ✅ 余裕   |
| 13B Q4  | 4096    | 1        | ✅ 安全   | ✅ 安全   |
| 13B Q4  | 8192    | 1        | ⚠️ 注意 | ✅ 安全   |
| 14B Q8  | 4096    | 1        | ⚠️ 注意 | ✅ 安全   |
| 27B Q4  | 4096    | 1        | ⚠️ 限界 | ✅ 安全   |
| 27B Q4  | 8192    | 1        | ❌ 不可   | ⚠️ 注意 |
| 32B Q4  | 4096    | 1        | ❌ 不可   | ⚠️ 限界 |
...
| モデル                       | サイズ  | 特徴                        | 推奨度 |
| ---------------------------- | ------- | --------------------------- | ------ |
| **LFM2.5 1.2B** (4-bit) | 約0.8GB | 超軽量・最速、Liquid AI製       | ⭐⭐⭐ |
| **Gemma 3 4B** (4-bit) | 約2.6GB | 軽量・高速、Vision対応 👁️     | ⭐⭐⭐ |
| **Qwen 3 4B** (4-bit)  | 約2.5GB | 軽量・日本語良好                | ⭐⭐⭐ |
| **MedGemma 4B** (8-bit) | 約1.2GB | 医療特化、Vision対応 👁️🏥    | ⭐⭐⭐ |
| Qwen 3 8B (4-bit)            | 約4.3GB | バランス良好                    | ⭐⭐   |
| Gemma 3 12B (4-bit)          | 約6.6GB | 高品質、メモリ余裕少            | ⚠️   |

**推奨**: `liquid/lfm2.5-1.2b`、`gemma-3-4b-it-4bit`、`medgemma-1.5-4b-it@8bit`、または `Qwen3-4B-4bit`（context: 2048〜4096）

#### 24GB以上 Mac（24GB / 32GB）

| モデル                        | サイズ  | 特徴                        | 24GB   | 32GB   |
| ----------------------------- | ------- | --------------------------- | ------ | ------ |
| **Gemma 3 12B** (4-bit) | 約6.6GB | 高品質、Vision対応 👁️     | ⭐⭐⭐ | ⭐⭐⭐ |
| **Qwen 3 8B** (4-bit)   | 約4.3GB | 日本語優秀、高速            | ⭐⭐⭐ | ⭐⭐⭐ |
| **gpt-oss 20B** (4-bit) | 約11GB  | OpenAI公式、MoE高効率       | ⭐⭐⭐ | ⭐⭐⭐ |
| **Qwen 3 14B** (4-bit)  | 約8-9GB | 高性能、日本語優秀          | ⭐⭐   | ⭐⭐⭐ |
| **Gemma 3 27B** (4-bit) | 約15GB  | 最高品質、Vision対応 👁️   | ⚠️   | ⭐⭐⭐ |
| Qwen 3 32B (4-bit)            | 約18GB  | 最高性能                    | ❌     | ⚠️   |

**24GB推奨**: `gemma-3-12b-it-4bit`、`Qwen3-8B-4bit`、または `gpt-oss-20b-4bit`（context: 4096〜8192）
**32GB推奨**: `gemma-3-27b-it-4bit`、`Qwen3-14B-4bit`、または `gpt-oss-20b-4bit`（context: 8192〜16384）

> **gpt-oss 20Bとは**: OpenAI初のオープンソースモデル（Apache 2.0）。21Bパラメータ中3.6Bのみ活性化するMoE構造で、メモリ効率が非常に高い。

### モデル選択ガイド

| 用途                         | 16GB Mac                            | 24GB以上 Mac                          |
| ---------------------------- | ----------------------------------- | ------------------------------------- |
| **画像認識（Vision）** | Gemma 3 4B 👁️ / MedGemma 4B 👁️🏥 / Qwen 3 VL 3B 👁️ | Gemma 3 12B〜27B 👁️ / Qwen 3 VL 8B 👁️ |
| **医療特化**           | MedGemma 4B 👁️🏥                  | MedGemma 4B 👁️🏥                    |
| **日本語重視**         | Qwen 3 4B / 8B                      | Qwen 3 8B / 14B（32GB: 14B / 32B）   |
| **軽量・高速**         | LFM2.5 1.2B / Gemma 3 4B            | Qwen 3 8B / Gemma 3 12B               |
| **高品質回答**         | Qwen 3 8B                           | gpt-oss 20B / Gemma 3 12B〜27B        |
...
| **MedGemma 4B** (8-bit) 👁️🏥 | ~1.2GB | 16GB | 医療特化、画像+テキスト |
...
> MLXはApple Silicon向けに最適化されたフォーマットで、GGUFより高速で効率的です。

#### Windows / Linux — GGUF形式

1. LM Studio → 探索（Ctrl+Shift+M）
2. 検索バー右側で **「GGUF」にチェック** ✓
3. `gemma 3` または `qwen 3` で検索
4. `Q4_K_M` 量子化バージョンを選択してダウンロード

> GGUFはクロスプラットフォーム対応のフォーマットです。量子化レベルの目安：
> - **Q4_K_M**: サイズ小・速度速（推奨）
> - **Q5_K_M**: バランス型
> - **Q8_0**: 高品質・サイズ大

**Windows での推奨モデル（GGUF Q4_K_M）:**

| VRAM          | 推奨モデル                              | context上限 |
| ------------- | --------------------------------------- | ----------- |
| **8GB**  | Gemma 3 4B / Qwen 3 4B / LFM2.5 1.2B   | 4096        |
| **12GB** | Qwen 3 8B / Gemma 3 12B                 | 4096〜8192  |
| **16GB+**| Gemma 3 12B / gpt-oss 20B / Qwen 3 14B  | 8192        |

> **注意**: WindowsではGPU VRAM（グラフィックメモリ）がボトルネックになります。NVIDIA GPUの場合、`nvidia-smi` でVRAMを確認してください。VRAMが不足する場合はCPUオフロードが発生し速度が低下します。
...
| **32GB** | 上記 + Gemma 3 27B / Qwen 3 14B                | 16384       |
...
- `localLLMChat_systemPromptPresets`: カスタムSystem Promptプリセット
- `localLLMChat_modelVisibility`: モデル表示フィルター設定
- `localLLMChat_sessions`: セッション管理データ
...
> **推奨**: Local LLM Chat v1.8.0 の全機能を使用するには **LM Studio v0.4.0以降** を使用してください。
...
- **新機能**
  - 🧠 **reasoning_effort パラメータ**: モデルの推論レベル（low / medium / high）を設定可能
  - 💭 **Thinking表示**: `<think>`タグによる思考プロセスの表示/折りたたみ切替
  - ⌨️ **キーボードショートカット一覧**: Ctrl+/ でショートカットモーダルを表示
  - 👁️ **モデル表示フィルター**: ドロップダウンに表示するモデルを選択可能
  - 📂 **セッション管理**: 複数の会話セッションを保存・切り替え
  - 👤 **ユーザー呼び名設定**: AIがユーザーを呼ぶ名前を設定可能
  - 💬 **System Promptプリセット**: 汎用アシスタント / 放射線科アシスタントを切替
- **UI改善**
  - 設定パネルを4タブ構成に整理（基本 / プロンプト / モデル / データ）
  - 「話題リセット」ボタン: 画面を保持したまま区切り線を表示、ユーザー情報を自動引き継ぎ
  - デフォルト System Prompt を汎用的な内容に変更
  - プリセットにプログラミング・文章作成・翻訳・ブレインストーミングを追加
  - エントリーファイル名を `index.html` に統一
- **改善**
  - MedGemma の `<medgemma_thinking>` タグに対応
  - Vision対応モデル判定に medgemma を追加
  - 推奨モデルに LFM2.5 1.2B、MedGemma 4B を追加
  - Windows (GGUF) ダウンロード手順を追加
