...
1. LM Studioを起動し、モデルを準備
   - 既にダウンロード済みの場合: 画面上部の「読み込むモデルを選択」から選択
   - 未ダウンロードの場合: 左側の🔍検索マーク（探索）からモデルを検索してダウンロード
2. LM Studioの「開発者」タブで「Local Server」を開始（Status: Runningにする）
3. 重要: Server Settingsで「CORSを有効にする」をONにする
...
2. Local Serverが開始されているか確認（Status: Running）
3. CORSが有効になっているか確認
4. 🔄 Refreshボタンをクリック
...
### 接続エラー
1. Local Serverが起動しているか確認
2. CORSが有効か確認
3. Base URLが正しいか確認

### 応答が途中で止まる・遅い
1. Max Tokensの値を確認（小さすぎると途中で切れる）
2. モデルサイズが大きすぎる可能性（小さいモデルを試す）
3. PCのメモリ/GPU負荷を確認
4. 長い会話は一度Clearしてリセット
...
### PDFが読み込めない
1. PDFサイズが10MB以下か確認
2. パスワード保護されたPDFは対応していない
3. 画像のみのPDF（スキャン）はテキスト抽出不可

### 日本語が文字化けする
- 添付ファイルの文字コードがUTF-8か確認
- UTF-8以外のファイルは文字化けする場合がある
...
### Q: 会話履歴を削除するには？
A: 🗑 Clearボタンで現在の会話をクリア。完全削除は設定パネルの「すべての保存データを消す」を使用。

### Q: 複数のモデルを同時に使えますか？
A: LM Studioで複数モデルをロードし、ドロップダウンで切り替え可能です。会話中でも切り替えられます。
...
### Q: 会話を他の人と共有できますか？
A: 💾 Exportで会話をJSONファイルとしてエクスポートし、共有できます。📥 Importで他の人からもらったJSONファイルを読み込むこともできます。
...
`.trim();
  ...
  function editUserMessage(msgDiv) {
    const msgContent = msgDiv.dataset.content || "";
    console.log("[DEBUG] editUserMessage - msgContent:", msgContent);
    console.log("[DEBUG] editUserMessage - messages:", messages);
    ...
    console.log("[DEBUG] editUserMessage - found idx:", idx);
    if (idx === -1) {
  ...
  function regenerateLastAssistant(msgDiv) {
    ...
    if (lastUserDiv) {
      ...
      console.log("[DEBUG] regenerate: found user message in UI:", userContent.substring(0, 50) + "...");
    }
    ...
    if (!userContent) {
      for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i].role === "user") {
          ...
          console.log("[DEBUG] regenerate: found user message in history:", userContent.substring(0, 50) + "...");
          break;
        }
      }
    }
...
上記のマニュアル内容を基に、ユーザーの質問に回答してください。`;
    ...
    while (true) {
      ...
      for (const ev of events) {
        ...
        try {
          ...
          // v1.6.7 デバッグ: logprobsの構造を確認
          if (j.choices?.[0]?.logprobs || j.choices?.[0]?.delta?.logprobs || j.logprobs) {
            console.log("[Logprobs Debug] Response structure:", JSON.stringify({
              hasChoicesLogprobs: !!j.choices?.[0]?.logprobs,
              hasDeltaLogprobs: !!j.choices?.[0]?.delta?.logprobs,
              hasTopLevelLogprobs: !!j.logprobs,
  ...
  async function handleSend() {
    ...
    console.log("[DEBUG] handleSend - using model:", model, "(select:", el.modelSelect.value, ", settings:", settings.model, ")");
    ...
    try {
      ...
      if (settings.showLogprobs && !conversationHasImage) {
        ...
        console.log("[Logprobs Debug] Using /v1/responses API for logprobs support");
        ...
        console.log("[Logprobs Debug] Request body:", JSON.stringify(responsesBody, null, 2));
        ...
        if (!res.ok) {
          ...
          console.error("[Logprobs Debug] API error:", res.status, t);
          return;
        }

        const responseData = await res.json();
        console.log("[Logprobs Debug] Response data:", responseData);
              ...
              for (const contentItem of item.content) {
                if (contentItem.type === "output_text") {
                  ...
                  if (contentItem.logprobs) {
                    console.log("[Logprobs Debug] Found logprobs in output_text:", contentItem.logprobs);
        ...
        console.log("[Logprobs Debug] Extracted content length:", content.length);
        console.log("[Logprobs Debug] Extracted logprobs count:", allLogprobs.length);
        ...
        if (allLogprobs.length > 0) {
          console.log("[Logprobs Debug] Calling displayLogprobsInfo with", allLogprobs.length, "entries");
          displayLogprobsInfo(currentMsgDiv, allLogprobs);
        } else {
          console.log("[Logprobs Debug] No logprobs in response. Check LM Studio version >= 0.3.39");
        }
        ...
        if (settings.showLogprobs && conversationHasImage) {
          console.log("[Logprobs Debug] Falling back to /v1/chat/completions because conversation contains images");
          console.log("[Logprobs Debug] Note: logprobs will not be available for this response");
        }
        const requestBody = {
          model,
...
【疾患名】`,
    ddx: `まず鑑別を5つ挙げ、自信度(高/中/低)と根拠を1行で示してください。
「見逃し厳禁」と、最後に普段は思いも付かない｢大穴｣診断を1つ追加。
...
【文章】`,
  ...
  function wireSettingsEvents() {
    ...
    el.modelSelect.addEventListener("change", (e) => {
      ...
      // 即座にsettings.modelを更新してから保存
      settings.model = id;
      save();
      console.log("[DEBUG] Model changed to:", id, "settings.model:", settings.model);
