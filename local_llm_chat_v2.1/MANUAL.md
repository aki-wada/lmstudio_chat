...
- **ユーザーメッセージ**: 右寄せ、落ち着いた青のグラデーション背景、白文字、角丸バブル
- **AIアシスタントメッセージ**: 左寄せ、半透明フロストガラス風背景、「🤖 Assistant」ラベル付き
...
- **詳細タブ**: 信頼度表示ON/OFF、モデル自動アンロードON/OFF、表示モデル管理、プリセット編集、データ管理
...
設定 → 詳細タブの「表示モデル管理」で、モデル選択ドロップダウンに表示するモデルを絞り込めます：

1. 「🔄 モデル一覧を取得」をクリックして全モデルを読み込み
2. 表示したいモデルにチェックを入れる（チェックを外すとドロップダウンから非表示に）
3. 「全選択」「全解除」で一括操作も可能

- フィルター設定はlocalStorageに保存され、次回起動時にも維持されます
- フィルター未設定時は従来通り全モデルが表示されます

多数のモデルをダウンロードしている場合に、よく使うモデルだけを表示して素早く選択できます。
...
- **表示モデル管理**: ドロップダウンに表示するモデルを選択（v2.1新機能）
...
- `localLLMChat_modelFilter`: 表示モデルフィルター設定
...
> **推奨**: Local LLM Chat v2.1 の全機能を使用するには **LM Studio v0.4.0以降** を使用してください。
...
- **新機能**: 表示モデル管理（モデルフィルター）
  - 設定 → 詳細タブで、ドロップダウンに表示するモデルを選択可能
  - 多数のモデルから必要なものだけを表示してすばやく選択
  - フィルター設定はlocalStorageに保存・復元
- **デザイン更新**: グラスモーフィズムUI（青ベース）
  - ブルーカラーパレット、フロストガラス効果のヘッダー・入力エリア・設定パネル
  - ユーザーバブル: 落ち着いた青グラデーション＋白文字
  - AIメッセージ: フロストガラス風半透明背景
  - フォント: SF Pro Text
- v2.0の全機能を継承
